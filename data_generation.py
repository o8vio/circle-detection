...
def random_circles(num_circles, world_dim=3, r_min=0.25, r_max=0.3):
    ...
    if world_dim > 2:
        ...
        bases = [ np.identity(world_dim) for _ in range(num_circles) ]
...
def generate_sample(num_circles, avg_points=200, std_points=10, world_dim=3, 
    ...
    num_signal_points = 0 if num_circles == 0 else round((1 - sigma) * num_points)
    num_noise_points = num_points - num_signal_points
    
    centers, radii, bases = random_circles(num_circles, world_dim, r_min, r_max)
    
    points = []
    for i, (center, radius, basis) in enumerate(zip(centers, radii, bases)):

        num_circle_points = num_signal_points // num_circles + (i < num_signal_points % num_circles)
    ...
    return centers, radii, bases, np.array(points)
