...
exp2_results = { feature+'_results' : { score_key : np.zeros(shape=(n_sigma, n_seeds)) for score_key in score_keys }
...
for i, sigma in enumerate(sigma_values):
  ...
  for j in range(n_seeds):
    ...
    tp = topopipeline.random(seed=j, samples_per_class=20, avg_points=250, std_points=15, 
                             r_min=0.25, r_max=0.3, eps=0.05, sigma=sigma)
    ...
    for feature_key in features:
      ...
      exp2_results[feature_key+'_results']['test_score'][i, j] = model.score(X_test, y_test)
      exp2_results[feature_key+'_results']['train_score'][i, j] = model.score(X_train, y_train)

      print(f'current param: {i+1}/{n_sigma} , current seed: {j+1}/{n_seeds}', end='\r', flush=True)

plot_scores(param_range=sigma_values, results=exp2_results,
            titles=[key+' results' for key in feature_keys],
            fill=True, y_label='accuracy', param_name='sigma', show=False)

plt.savefig('./experiments/results/exp2_results.pdf')

save_results(exp2_results, './experiments/results/exp2_results.json')